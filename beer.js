...
console.log(`🍻 Aquí las cervezas:`, beers);

// 2. Generar una funcion que reciba como parametro el array de cervezas y un valor de alcohol (numero). La funcion debe devolver un nuevo array con las cervezas que no excedan el nivel etílico. Cada elemento del nuevo array debe ser un objeto que tenga sólo las propiedades name, (alcohol) abv y ("amargor") ibu.
const obtenerCervezas = (array, valorAlcohol) => {
  const filtrarCervezas = array.filter((cerveza) => cerveza.abv < valorAlcohol);

  const nuevoArrayCervezas = filtrarCervezas.map((cerveza) => {
    return {
      name: cerveza.name,
      abv: cerveza.abv,
      ibu: cerveza.ibu,
  ...
  return nuevoArrayCervezas;
};
const arrayMenosEtilico = obtenerCervezas(beers, 5)
console.log(`🍻 Aquí las cervezas que NO exceden el nivel etílico: `, arrayMenosEtilico);

// 3. Generar una función que reciba como parámetro un array de cervezas y devuelva un nuevo array con las 10 cervezas más alcohólicas
const obtenerCervezasMasAlc = (array) => {
  const ordenarCervezas = array.sort((a, b) => b.abv - a.abv);
  const cervezasMasAlcoholicas = ordenarCervezas.filter(
    (cerveza) => cerveza.abv > 7.3
  );
  return cervezasMasAlcoholicas;
};
const arrayMasAlcoholico = obtenerCervezasMasAlc(beers)
console.log(`🍻 Aquí las cervezas MÁS alcoholicas: `, arrayMasAlcoholico); //USAR SPLICE

// 4. Generar una función que reciba como parámetro un array de cervezas y devuelva un nuevo array con las 10 cervezas menos amargas.
const obtenerCervezasMenosAmarg = (array) => {
  const copiaCervezas = [...array];
  const ordenarCervezas = copiaCervezas.sort((a, b) => b.ibu - a.ibu);
  const cervezasMenosAmargas = ordenarCervezas.splice(0, 10);
  return cervezasMenosAmargas;
};
const arrayMenosAmargo = obtenerCervezasMenosAmarg(beers)
console.log(`🍻 Aquí las cervezas MENOS amargas: `, arrayMenosAmargo);

// 5. Generar una función que reciba como parámetro un array de cervezas un nombre de propiedad y un valor booleano. Debe devolver un nuevo array con 10 cervezas ordenadas por la propiedad ingresada como segundo argumento de manera ascendente si el tercero es true o descendente si es false
const ordenarArrayCervezas = (array, propiedad, valorBool) => {
  if (typeof propiedad === "number") {
    if (valorBool === true) {
      const copiaCervezasAsc = [...array];
      const cervezasAscendente = copiaCervezasAsc.sort(
        (a, b) => b[propiedad] - a[propiedad] //Cuando se pasan por parametros en vez de que este establecida de forma estática, se deben usar corchetes.
      );
      const cervezasOrdenadasAsc = cervezasAscendente.splice(0, 10);
      return cervezasOrdenadasAsc;
    }
    if (valorBool === false) {
      const copiaCervezasDesc = [...array];
      const cervezasDescendente = copiaCervezasDesc.sort(
        (a, b) => a[propiedad] - b[propiedad]
      );
      const cervezasOrdenadasDesc = cervezasDescendente.splice(0, 10);
      return cervezasOrdenadasDesc;
    }
  }
  if (typeof propiedad === "string") {
    if (valorBool === true) {
      const copiaCervezasAsc = [...array];
      const cervezasAscendente = copiaCervezasAsc.sort(
        (a, b) => a[propiedad].localeCompare(b[propiedad])
      );
      const cervezasOrdenadasAsc = cervezasAscendente.splice(0, 10);
      return cervezasOrdenadasAsc;
    }
    if (valorBool === false) {
      const copiaCervezasDesc = [...array];
      const cervezasDescendente = copiaCervezasDesc.sort(
        (a, b) => b[propiedad].localeCompare(a[propiedad])
      );
      const cervezasOrdenadasDesc = cervezasDescendente.splice(0, 10);
      return cervezasOrdenadasDesc;
    }
...
//ordenarArrayCervezas(beers, "abv", true);
//ordenarArrayCervezas(beers, "abv", false);

const stringAscendente = ordenarArrayCervezas(beers, "name", true);
console.log(`🍻 Aquí las cervezas ASCENDENTES según su nombre`, stringAscendente)

const stringDescendente = ordenarArrayCervezas(beers, "name", false);
console.log(`🍻 Aquí las cervezas DESCENDENTES según su nombre`, stringDescendente)